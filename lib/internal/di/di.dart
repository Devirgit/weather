...
class Di {
  ...
  static final Map<String, ValueGetter<dynamic>> _builders =
      <String, ValueGetter<dynamic>>{};

  static String _generateDiCode<T>([String name = '']) {
    return '$T$name';
  }

  static void reg<T>(ValueGetter<T> builder,
      {String name = '', bool asBuilder = false}) {
  ...
  static bool reset<T>({String name = '', Function(T? object)? dispose}) {
    final String code = _generateDiCode<T>(name);

    if (!_dependencies.containsKey(code) && !_builders.containsKey(code)) {
      return false;
    } else if (_dependencies.containsKey(code)) {
      final object = _dependencies.remove(code);
      if (dispose != null) {
        dispose(object);
      }
    } else {
      _builders.remove(code);
    }
